...
def predict_for_flight(model, loader, device):
    ...
    y_true = torch.cat(targets_all, dim=0) # (N, V, F)
...
def inverse_scale_predictions(y_pred, y_true, scaler):
    ...
    y_pred_real = y_pred_real.reshape(N, F, V).transpose(0, 2, 1)
    y_true_real = y_true_real.reshape(N, F, V).transpose(0, 2, 1)
...
def compute_mae_for_flight(
    ...
    mae_xyz = abs_error.mean(axis=(0, 2))  # (V,)
...
def compute_ade_for_flight(
    ...
    l2_dist_first = np.linalg.norm(disp[:, :, 0], axis=1)
...
def plot_3d_trajectory(y_pred_real, y_true_real, forecast_step=0):
    ...
    x_true = y_true_real[:, x_idx, forecast_step]
    y_true = y_true_real[:, y_idx, forecast_step]
    z_true = y_true_real[:, z_idx, forecast_step]

    x_pred = y_pred_real[:, x_idx, forecast_step]
    y_pred = y_pred_real[:, y_idx, forecast_step]
    z_pred = y_pred_real[:, z_idx, forecast_step]
